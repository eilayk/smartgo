use crate::models::{
    get_trip_number_from_id, AppError, AppState, RouteSearch, Stop, StopTime, StopTimeResponse,
    Time, TimeQuery,
};
use axum::{
    debug_handler,
...
pub async fn get_stop_times(
    ...
    for row in rows.iter() {
        ...
        if arrival_time.is_relevant_to(&time.time) {
            ...
            app_state
                .api
                .get_next_service(&stop_id, &route_id, &trip_number)
                .await
                .map(|line| {
                    ...
                    if line.computed_departure_time.time != result.arrival_time {
                        result.actual_arrival_time =
                            Some(line.computed_departure_time.time.clone());
                    }
            ...
            app_state
                .api
                .get_service_at_a_glance(trip_number)
                .await
                .map(|trip| {
